...
def calculate_transition_matrix(trajectories,topic_labels):
    ...
    traj = [traj[:-1] for traj in trajectories]
    traj = np.concatenate(traj)
    traj_lag = [traj[1:] for traj in trajectories]
    traj_lag = np.concatenate(traj_lag)


    traj = traj.astype(np.float)
    traj_lag = traj_lag.astype(np.float)
    #dtraj_fuzzy = preprocessing.normalize(traj,axis=1,norm="l1")
    #dtraj_fuzzy_lag = preprocessing.normalize(traj_lag,axis=1,norm="l1")
    C_00 = traj.T @ traj
    C_01 = traj.T @ traj_lag

    null_columns=list(set(np.where(C_01.sum(0)<2)[0].tolist()+np.where(C_00.sum(0)<2)[0].tolist()))
    null_columns_to_delete = null_columns=list(set(np.where(C_01.sum(0)<2)[0].tolist()+np.where(C_00.sum(0)<2)[0].tolist()))


     # Remove null columns
    while len(null_columns_to_delete)>0:
        for d,null in enumerate(null_columns[::-1]):
            try:
                C_01 = np.delete(C_01,null, 0)  
                C_01 = np.delete(C_01,null, 1)
                C_00 = np.delete(C_00,null, 0)  
            
                C_00= np.delete(C_00,null, 1)
            except:
                pdb.set_trace()
            try:
        ...
        null_columns=list(set(np.where(C_01.sum(0)==0)[0].tolist()+np.where(C_00.sum(0)==0)[0].tolist()))[::-1]
        null_columns_to_delete = null_columns=list(set(np.where(C_01.sum(0)==0)[0].tolist()+np.where(C_00.sum(0)==0)[0].tolist()))
    ...
    minus = transition_matrix_fuzzy[ np.where( transition_matrix_fuzzy <0 )].shape[0]
    plus = transition_matrix_fuzzy[ np.where( transition_matrix_fuzzy >+0 )].shape[0]
    print ("ratio of minus values to 0 or bigger")
    print (minus/plus)
    print (transition_matrix_fuzzy[transition_matrix_fuzzy<0].shape)
...
def process_data(data_set):
    ...
    trajectories = np.take(segment_keyword_matrix,data_set.index.to_list(),axis=0)
...
if __name__ == '__main__':
    ...
    import json
    with open(constants.output_data_features+'segment_keyword_matrix_from_preprocessed_data.json') as json_file:
        segment_keyword_matrix = json.load(json_file)
    segment_keyword_matrix = np.array(segment_keyword_matrix)
